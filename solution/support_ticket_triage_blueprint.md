# Support Ticket Triage Agent — Final Blueprint v3.5 (Aligned with Course_materials.md, Memory-Complete, Build-Ready)
...
| v3.5 | Aligned with `Course_materials.md`: added Executive Map, message-protocol & `MessagesPlaceholder` guidance, context-engineering patterns, memory reducer clarifications, forced-tool-calling guidance, recommended dependency pins, and optional LangSmith observability notes. |

## 0. Course Materials Alignment (summary of applied updates)

This blueprint (v3.5) was updated using the concise implementation guidance from `Course_materials.md`. The changes below are intentionally focused and actionable so the notebook implementation remains Colab-safe and submission-ready.

- Executive Map applied: clear layering from stateless model → message protocol → context engineering → tools/retrievers → agents → LangGraph orchestration → memory → safety → observability.
- Message protocol: adopt `SystemMessage` / `HumanMessage` / `AIMessage` / `ToolMessage` roles and prefer `MessagesPlaceholder` for dynamic history injection to simplify trimming and summarization.
- Context engineering: explicitly call out the read-before-act pattern, top-k retrieval injection, and short-window + summarization memory controls before every LLM call.
- Tools: emphasize narrow, schema'd tools with docstrings and support for forced-tool-calling when deterministic business rules apply (e.g., order lookups, account reads).
- Memory: clarify reducer patterns (use `Annotated[list, add_messages]` or explicit append semantics) and persistent namespaces in SQLite; maintain strict separation between short-term messages, long-term facts, and RAG KB content.
- Guardrails & HITL: layered checks (deterministic filters before LLM, model-based judge after LLM, and a mandatory HITL gate before critical side-effects) are now explicit in node patterns.
- Observability: note LangSmith as optional but recommended — baseline keeps the submission minimal, but enabling LangSmith via environment variables is documented in the README section.
- Tests & evaluation: mirror Course_materials' recommendation to convert traces into golden datasets and add evaluator-based regression tests (LLM-as-judge if permitted).

How this affects the notebook:
- Minor prompt refactors to use `MessagesPlaceholder` and structured output (JSON schema) for key agents.
- Add a small `prompt_helpers` cell demonstrating trimming + placeholder usage.
- Add a short `observability` cell explaining how to enable LangSmith only when allowed.
